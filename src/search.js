...
function Search(props) {
  ...
  const fetchQuote = async () => {
    const res = await fetch(
      `http://localhost:3000/api/v1/portfolio/search/${inputText}`
    );
    let json = await res.json();
    console.log(json);
      ...
      <div className="grid grid-cols-2">
        <div className={"border p-5"}>
          <input
            ...
            type="text"
            className={"border w-full p-3 rounded-full border-gray-300"}
          />
        </div>
        <div className={"border p-5"}>
          <span
            onClick={fetchQuote}
            className={
              "bg-gray-600 cursor-pointer p-2 rounded text-white text-xl pl-5 pr-5"
            }
          >
            Get Quote
          </span>
